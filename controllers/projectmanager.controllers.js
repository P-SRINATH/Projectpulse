...
const getProjectConcerns = expressAsyncHandler(async (req, res) => {
  let manager = await Projects.findAll({
    where: { project_manager_email: req.params.email },
  });
  let project = manager.map((projects) => projects.dataValues.project_id);
  let disdata = await sequelize.query(
    "select * from project_concerns where project_id in(?)",
    {
...
const getProjectUpdates = expressAsyncHandler(async (req, res) => {
  ...
  let project = manager.map((projects) => projects.dataValues.project_id);
  let disdata = await sequelize.query(
    "select * from project_updates where project_id in(?)",
    {
      replacements: [project],
    }
  );
...
const getProjectsByManager = expressAsyncHandler(async (req, res) => {
  let projectbymanager = await Projects.findAll({
    where: { project_manager_email: req.params.email },
  });
  ...
  if (projectbymanager == undefined) {
    ...
    res.status(200).send({
      message: "The projects Under Project Manager are : ",
      projects: projectbymanager,
    });
  }
});
...
const updateProjectConcerns = expressAsyncHandler(async (req, res) => {
  let getprojectconcern = await ProjectConcerns.findOne({
    where: { id: req.params.id },
  });
  if (getprojectconcern == undefined) {
    res.status(404).send({
      message: "There is no Project Concern raised with the given id ",
    });
  } else {
...
const updateProjectUpdates = expressAsyncHandler(async (req, res) => {
  let getprojectupdate = await ProjectUpdates.findOne({
    where: { id: req.body.id },
  });
  if (getprojectupdate == undefined) {
    res
      .status(404)
      .send({ message: "There is no Project-Update available to modify " });
  } else {
    await ProjectUpdates.update(
      {
        project_status_update: req.body.project_status_update,
        waiting_for_client: req.body.waiting_for_client,
      },
      { where: { id: req.body.id } }
    );
...
const getSpecificProjectDetails = expressAsyncHandler(async (req, res) => {
  let allprojects = await Projects.findOne({
    where: {
      project_manager_email: req.params.email,
  ...
  res.status(200).send({
    message: `Project ${req.params.project_id} under the project manager with email ${req.params.email}:`,
