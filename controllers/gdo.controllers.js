...
const addProjectTeam=expressAsyncHandler(async(req,res)=>{
    ...
    let verifyEmployee=await Employee.findOne({where:{employee_email:req.body.resource_name}})
    if(verifyEmployee==undefined){
        res.status(404).send({message:"Resource not a member of the organization"})
    }
    else{
...
const getProjectPortfolioDashboard=expressAsyncHandler(async(req,res)=>{
    ...
    let portfolio=await Projects.findAll({where:{gdoemail:req.params.email},
        attributes:{
           exclude:['domain_of_project','type_of_project','gdoemail','project_manager_email']
        }
    })
    res.status(200).send({message:"Project-Portfolio-Dashboard",projects:portfolio})
})
...
const getProjectDetails=expressAsyncHandler(async(req,res)=>{
    let allprojects=await Projects.findOne({where:{gdoemail:req.params.email,project_id:req.params.project_id},
  ...
  res.status(200).send({message:"Project under the GDO are: ",ProjectFitness:projectFitness,TeamMembers:teamSize,ConcernIndicator:concernindicator,projects:allprojects})
